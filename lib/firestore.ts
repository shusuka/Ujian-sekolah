          ...
          if (snap.exists()) {
            ...
            const data: Question[] = typeof raw === "string" ? JSON.parse(raw) : raw;
                  if (data && data.length > 0) {
                            // Merge imageUrl & chartData from local bank in case Firestore data was seeded
                    // before those fields were added (legacy seed issue)
                    return mergeLocalVisuals(data, subjectId, sessionNumber);
                  }
          }
    } catch (err) {
...
/**
 * Merges imageUrl and chartData from local question bank into Firestore questions.
 * This fixes questions that were seeded before visual fields were added.
 * Matching is done by question id (same index logic as buildQuestionsFromBank).
 */
function mergeLocalVisuals(
    firestoreQuestions: Question[],
    subjectId: string,
    sessionNumber: number
  ): Question[] {
    const localQuestions = getLocalQuestions(subjectId, sessionNumber);
    const localMap = new Map(localQuestions.map((q) => [q.id, q]));
    return firestoreQuestions.map((q) => {
          const local = localMap.get(q.id);
          if (!local) return q;
          const merged: any = { ...q };
          if (!merged.imageUrl && (local as any).imageUrl) {
                  merged.imageUrl = (local as any).imageUrl;
          }
          if (!merged.chartData && (local as any).chartData) {
                  merged.chartData = (local as any).chartData;
          }
